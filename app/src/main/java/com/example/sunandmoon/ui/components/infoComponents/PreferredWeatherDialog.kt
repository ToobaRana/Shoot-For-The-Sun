    ...
    AlertDialog(
        ...
        text = {
            Column() {
                ...
                Text(text = stringResource(id = R.string.YourPreferredWeather), fontWeight = FontWeight.Bold)
                if(shoot.preferredWeather.isNotEmpty()) {
                    Column(modifier.fillMaxWidth(), horizontalAlignment = Alignment.CenterHorizontally) {
                        PreferredWeatherOverview(modifier, shoot.preferredWeather)
                    }
                }
                else Text(text = stringResource(id = R.string.NoPreferredWeatherRegistered), fontWeight = FontWeight.Bold, modifier = modifier.padding(0.dp, 10.dp))

                if( weatherIcon != null) {
                    Text(text = stringResource(id = R.string.ActualWeather), fontWeight = FontWeight.Bold)
